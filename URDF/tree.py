...
def create_joint_element(joint_id, parent_link, child_link,position):
    ...
    origin = ET.SubElement(joint, "origin", xyz=f"{position[0]} {position[1]} {position[2]}", rpy=f"0 0 {orientation}")
...
def generate_urdf(num_cubes):
    ...
    for i in range(1, num_cubes):
        parent_id = (i - 1) // 2
        cube_position = (random.uniform(-0.5, 0.5), random.uniform(-0.5, 0.5), random.uniform(0.3, 0.8))
