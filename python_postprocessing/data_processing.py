...
plt.plot(df['Time_diff'].values, df["Number of visible nodes"].values, label='Number of Visible Nodes')
plt.plot(df['Time_diff'].values, df["number of matched nodes"].values, label='Number of Matched Nodes')
plt.xlabel('Time (seconds)')
plt.ylabel('Count')
plt.title('Number of Visible and Matched Nodes Over Time')
plt.legend()
plt.xlim(0, df['Time_diff'].max())
plt.ylim(0, max(df[["Number of visible nodes", "number of matched nodes"]].values.max()+1, 0))
plt.grid()
