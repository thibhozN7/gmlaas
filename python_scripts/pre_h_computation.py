...
class PreHMatrix:
    ...
    def callback(self,m_isomorphism_list_sub,m_graph_sub,m_tag_sub):
        ...
        self.isomorphism_list = m_isomorphism_list_sub.data
        ...
        if self.timestamp != m_tag_sub.header.stamp:
            print("Timestamps do not match")
            return 0
        
        # Read the reference index matrix from the desired tags dataset
        ...
        # Build dictionaries for desired and current tag coordinates
        with open(f"{package_dir}/datasets/desired/tags_dataset.csv", "r") as file:
            reader = csv.reader(file)
            next(reader)  # Skip the header row
            self.desired_index_matrix
            for row in reader:
                tag_id =int(row[2])
                x = float(row[3])
                y = float(row[4])
